...
export function createAccountHandler(store: AccountStore) {
  return async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      if (raw.length > 16000) throw new RequestError(413, "Request is too large.");
      ...
      if (action === "access") return json({ isAdmin });
      ...
      if (action === "list") {
        ...
        return json(await store.directory((body.search as string | undefined)?.trim() ?? "", page(body.page)));
      }
      ...
      if (action === "details") return json(await store.details(targetId, page(body.page)));
      ...
      return json({ error: "The account operation could not be completed. Please retry." }, 500);
    }
  };
}
