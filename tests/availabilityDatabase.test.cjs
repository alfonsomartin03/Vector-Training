...
test("availability migration enforces ownership, validation, rollover ordering and cascade deletion", async () => {
  ...
  try {
    ...
    await db.exec(readFileSync(path.resolve(__dirname, "../supabase/migrations/20260926090000_availability_ranges.sql"), "utf8"));
    ...
    await db.query(insert, [one, "2026-09-28", 180, [0,2,4]]);
    await assert.rejects(db.query(insert, [two, "2026-09-21", 240, [0]]), /row-level security/);
    for (const [date, minutes, rest] of [["2026-09-22",240,[0]], ["2026-10-05",179,[0]], ["2026-10-05",2101,[0]], ["2026-10-05",240,[0,0]], ["2026-10-05",240,[0,1,2,3,4,5,6]], ["2026-10-05",240,[7]], ["2026-10-05",240,[null]]]) await assert.rejects(db.query(insert, [one,date,minutes,rest]), /check constraint/);
    ...
    assert.equal((await db.query("update training_availability set weekly_minutes = 180 returning *")).rows.length, 0);
